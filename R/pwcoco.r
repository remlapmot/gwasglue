...
gwasvcf_to_pwcoco <- function(vcf1, vcf2, chrompos, type1=NULL, type2=NULL, outfile)
{
	...
	utils::write.table(tib1, file=paste0(outfile, "1.txt"), row=F, col=T, qu=F)
	utils::write.table(tib1, file=paste0(outfile, "2.txt"), row=F, col=T, qu=F)
	return(0)
}
...
ieugwasr_to_pwcoco <- function(id1, id2, chrompos, type1=NULL, type2=NULL, outfile)
{
	...
	utils::write.table(tib1, file=paste0(outfile, "1.txt"), row=F, col=T, qu=F)
	utils::write.table(tib2, file=paste0(outfile, "2.txt"), row=F, col=T, qu=F)
	return(0)
}
